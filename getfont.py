...
__author__ = "d.kavraal@gmail.com"
__version__ = "0.0.1"
...
try:
	...
	print("You should install library: requests. Try:\n\tpip install requests\n")
	exit(-1)

def getFontUrls(family, subset, style):
	return getFontUrlsWithUserAgent(family, subset, style, AGENT_STRS["ttf"])

def getFontUrlsForFormat(family, subset, style, type):
	return getFontUrlsWithUserAgent(family, subset, style, AGENT_STRS[type])
...
if __name__ == "__main__":
	if len(sys.argv) < 2 or len(sys.argv) > 5:
		...
		if (format == "all"):
			fonts = []
			for format_i in AGENT_STRS.keys():
				fonts += getFontUrlsForFormat(family, subset, style, format_i)
			print('\n'.join(fonts))
			exit(0)
		else:
			print('\n'.join(getFontUrlsForFormat(family, subset, style, format)))
			exit(0)
